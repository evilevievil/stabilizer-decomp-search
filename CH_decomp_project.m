    ...
    if det(G) ~= 0
        ...
        for i = 1:decomp_len
            for j = 1:a_len
                ...
                a_stab_array(i) = a_stab_array(i)  + a(j) * CH_basis_inner_product(uint8(j-1),stab_array(i));
                %disp(a_stab_array);
            end
        end
        a_stab_array_conj = conj(a_stab_array);
        ...
        for i = 1:decomp_len
            for j = 1:decomp_len
                projection = projection + a_stab_array_conj(i) * a_stab_array(j) * G_inv(i,j);
            end
        end
    else
